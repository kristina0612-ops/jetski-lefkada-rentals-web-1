...
const ALLOWED_JETSKI_IDS = ["nero-ena", "nero-dio", "nero-tria", "nero-tessera"] as const;
...
export function validateBookingBody(body: unknown): Result<BookingInput> {
  ...
  if (!isInt(body.duration_minutes, 10, 1440)) return { ok: false, error: "duration_minutes out of range" };
  if (!isEnum(body.jetski_id, ALLOWED_JETSKI_IDS)) return { ok: false, error: "jetski_id not allowed" };
  ...
  return {
    ...
    data: {
      ...
      jetski_id: body.jetski_id as string,
      ...
      customer_email: (body.customer_email as string).trim().toLowerCase(),
      ...
      total_price: body.total_price as number,
      deposit_amount: body.deposit_amount as number,
