# Entscheidungen & Regeln – Jetski Lefkada Rentals

Dieses Dokument ist die **einzige autoritative Quelle** für Preise, Regeln und Bedingungen.
Claude darf NIEMALS Preise oder Regeln erfinden, die hier nicht stehen.
Änderungen nur nach Freigabe durch David oder Kristina.

---

## Flotte & Preisliste (Saison 2026)

Stand 2026-04-17: Flotte reduziert auf nur Sea-Doo (Yamahas ausgemustert/verkauft).

| Name           | Marke    | Modell         | PS  | Sitze | €/Stunde | €/Halbtag (4h) | €/Ganztag (8h) |
|----------------|----------|----------------|-----|-------|----------|----------------|----------------|
| The Challenger | Sea-Doo  | RXT-X 300      | 300 | 3     | €140     | €420           | €690           |
| The Acrobat    | Sea-Doo  | Spark Trixx    | 90  | 2     | €70      | €210           | €350           |

**Zusatzangebot: Water Fun / Towable** – 3-Reifen-Tubing hinter dem Jetski, **€30 pro Person** (Dauer ca. 10 Min.).

**Anzahlung:** 30% bei Buchung, Rest am Tag selbst.
**Zahlungsmethoden:** Viva Wallet, Bar (Euro), Karte.

---

## Neue Preiskategorien (Flyer 2026)

Ergänzend zu den Stunden-/Halbtag-/Ganztag-Preisen oben führt der Flyer 2026 folgende Kategorien ein. **Stand 2026-04-18:** Diese Preise gelten für **beide** Jetskis identisch (Challenger und Acrobat kosten gleich in diesen Kategorien). Die Classic-Preise oben bleiben pro Jetski unterschiedlich.

### Beach Rides (kurze Fahrten ab Dock) – pro Jetski gleich
- 10 Min – **80 €**
- 15 Min – **90 €** („BEST")
- 20 Min – **100 €**
- 30 Min – **130 €**
- 60 Min – **200 €** („BESTSELLER")

### Exclusive Experiences – pro Jetski gleich
- **Sunset Ride** (30 Min) – **130 €**
- **Couple Ride** (30 Min) – **150 €**

### VIP Delivery Service (Lieferung an Strand/Boot/Yacht/Katamaran) – pro Jetski gleich
- 1 Stunde – **350 €**
- Halbtag (4 h) – **450 €**
- Ganztag (8 h) – **650 €**
- Wochenpreis – **auf Anfrage**
- **Kaution:** 1.500 € (nur bei Delivery)
- **Kraftstoff NICHT inklusive**

### Towable / Water Fun
- 10 Min – **30 € pro Person**
- Tube-Modell: **Jobe Ridge III** (3 Sitze)

---

## Buchungsregeln

- **Mindestalter:** 18 Jahre (Lichtbildausweis Pflicht)
- **Bootsführerschein:** Nicht erforderlich
- **Briefing:** 10 Minuten Safety-Briefing am Dock – Pflicht für alle Fahrer
- **Treffpunkt:** Lygia Port, Lefkada (GPS: 38.7893° N, 20.7192° E – wird bei Buchungsbestätigung mitgeschickt)
- **Öffnungszeiten:** 10:00–13:00 und 16:00–19:00 Uhr, Mai bis September

---

## Stornierungsbedingungen

| Zeitraum vor Fahrt       | Rückerstattung |
|--------------------------|----------------|
| Mehr als 48 Stunden      | 100% (kostenlos) |
| 24 bis 48 Stunden        | 50%            |
| Weniger als 24 Stunden   | Keine Rückerstattung |
| Wetter-Absage (Beaufort 5+) | 100% ODER kostenlose Umbuchung |

---

## Versicherung

- **Haftpflicht:** €700.000 pro Vorfall
- **Gesetzliche Grundlage:** Griechisches Gesetz 4926/2022
- **Unfallversicherung für Fahrer:** Nicht inkludiert – wird empfohlen
- **Kaution:** Schadensfreiheitskaution kann verlangt werden (Betrag: wird ergänzt)

---

## Wetter-Regeln

- **Beaufort 5 oder mehr:** Betrieb eingestellt (griechische Küstenwache-Vorschrift)
- **Entscheidung:** David trifft die Entscheidung morgens
- **Kundenmitteilung:** Kunden werden sofort per WhatsApp/E-Mail informiert
- **Rückerstattung bei Wetter-Absage:** 100% oder kostenlose Umbuchung

---

## Sicherheitsregeln

- **Maximale Entfernung vom Ufer:** 700 Meter (Küstenwache-Vorschrift, Griechenland)
- **Schwimmzonen:** Keine Einfahrt in durch Bojen markierte Schwimmzonen
- **Schwimmweste:** Pflicht für alle Fahrer (wird gestellt)
- **Alkohol:** Fahren unter Alkohol-/Drogeneinfluss verboten – führt zu sofortigem Abbruch ohne Rückerstattung

---

## Schadensfälle

1. Schadensprotokoll erstellen (mit Claude)
2. Fotos machen (Handy)
3. Versicherung informieren (Nummer: wird ergänzt)
4. Bei Verletzung: **112** rufen (Notruf Griechenland)
5. Küstenwache: **108**

---

## Notfallkontakte

| Situation           | Kontakt            |
|---------------------|--------------------|
| Notruf allgemein    | 112                |
| Küstenwache         | 108                |
| David (WhatsApp)    | +30 695 561 2777   |
| Polizei Lefkada     | 26450 22100        |

---

## Was Claude bei Preisfragen tun muss

1. Nur Preise aus dieser Datei nennen
2. Wenn gefragt nach Rabatten: IMMER Kristina/David fragen, nie selbst entscheiden
3. Wenn Preisinformation fehlt: „Das weiß ich nicht genau – frag David"
4. Preise NIE abrunden, erhöhen oder schätzen
